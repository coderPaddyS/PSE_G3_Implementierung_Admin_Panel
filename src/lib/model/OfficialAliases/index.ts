...
export type OfficialAliasesListener = (table: Table<string>) => void;
 ...
 * @version 0.0.1
 */
export class OfficialAliasses {
    protected data: Alias[];
    protected table: Table<string>;
    protected listeners: Set<OfficialAliasesListener>;
     ...
     */
     private fromArray(data: Alias[]): Table<string> {
        ...
        table.setTitle(new TitleRow<string>().add(
            new TitleCell<string>(this.sorter).set(new TableData<string>("Alias")),
    ...
    public addListener(update: OfficialAliasesListener) {
